...
不要求每一帧都显示全部 14 个点，但原仓库的预训练模型不是为“离地 40 cm、只看半场”的相机专门训练的，因此不能仅凭 README 的完整球场指标判断可用。

从其单应性代码看，候选配置需要至少 4 个非退化的有效点才能初始化单应矩阵；其余点可以由球场模板和单应性补出。因此，半场画面在下面条件成立时有机会工作：

- 至少能看到 4 个有明确语义、且不接近共线的球场交点；
- 这些点覆盖的区域足够大，最好同时包含底线/边线/发球线等不同方向的几何约束；
- 能通过左右半场、近远端等语义消除镜像或错误配置；
- 相机内参和畸变已经正确处理。

如果只看到一小块球场、只看到一条边线，或 4 个点几乎共线，则不能稳定估计单应性，更不能可靠恢复相机高度和完整姿态。低相机还会带来球网、围栏、球员遮挡和强烈的近处透视变化，使公开转播数据的域差异更明显。

因此，对本项目的实际建议是：把该仓库用于验证网络接口和关键点定义，但采集机器人相机视角的数据进行微调；训练标签增加 `visible`，并在推理端只用可见且高置信度的点做 RANSAC/位姿优化。若长期只看固定半场，也可以只训练该半场对应的关键点集合和线段 mask，通常会比强行预测完整 14 点更可靠。
