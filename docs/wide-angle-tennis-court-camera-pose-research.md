...
- 普通广角、畸变较小：先做离线内参标定和去畸变，再使用针孔模型。
- 鱼眼或超广角：保留鱼眼/等距投影模型，直接在畸变图像上做重投影优化；简单地把边缘裁掉或强行套普通单应性会损失有效视野。
...
4. 用项目相机做一次棋盘格或 Charuco 鱼眼内参标定，把“镜头畸变误差”和“球场检测误差”分开测。
5. 在 H 稳定后，再做已知内参下的位姿分解和鱼眼重投影优化。
